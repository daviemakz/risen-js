...
var validateHttpOptions = function validateHttpOptions(httpOptions) {
  return httpOptions.every(function (http) {
    switch (true) {
      ...
      case http.hasOwnProperty('routes') && !Array.isArray(http.routes) || !http.routes.every(function (route) {
...
var validateCoreOperations = function validateCoreOperations(options) {
  return options instanceof Object ? Object.entries(options).every(function (_ref) {
...
var validateOptions = function validateOptions(options) {
  switch (true) {
    ...
    case options.hasOwnProperty('microServiceConnectionTimeout') && typeof options.microServiceConnectionTimeout !== 'number':
      {
        throw new Error('The "microServiceConnectionTimeout" option is not valid, it must be a number');
      }

    case options.hasOwnProperty('microServiceConnectionAttempts') && typeof options.microServiceConnectionAttempts !== 'number':
      {
    ...
    case options.hasOwnProperty('runOnStart') && !Array.isArray(options.runOnStart) || Array.isArray(options.runOnStart) && options.runOnStart.length && options.runOnStart.every(function (op) {
...
var validateServiceOptions = function validateServiceOptions(serviceOption) {
  switch (true) {
    case serviceOption.hasOwnProperty('runOnStart') && !Array.isArray(serviceOption.runOnStart) || Array.isArray(serviceOption.runOnStart) && serviceOption.runOnStart.some(function (op) {
      return typeof op !== 'string';
    }):
      {
    ...
    case serviceOption.hasOwnProperty('loadBalancing') && !['random', 'roundRobin'].includes(serviceOption.loadBalancing):
      {
    ...
    case serviceOption.hasOwnProperty('instances') && (typeof serviceOption.instances !== 'number' || typeof serviceOption.instances === 'number' && serviceOption.instances < 1):
      {
