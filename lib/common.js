    ...
    const sendDataToGateway = () => {
      ...
      if (Object.values(portSpeaker.sockets).length === 0) {
        ...
        if (_connectionAttempts <= this.settings.connectionTimeout) {
          ...
          this.log('NOTICE: API Gateway Server Socket Has Not Initialized Yet...', 'log');
          return setTimeout(() => {
            sendDataToGateway();
        ...
        // Notification
        this.log(`ERROR: Unable To Connect To API Gateway Server. MORE INFO: ${resBody.destination}`, 'log');
        ...
        portSpeaker.request('COM_REQUEST', resBody, dataR => {
          ...
          if (dataR.hasOwnProperty('error')) {
            // Notification
            this.log(`ERROR: Unable To Connect To Micro Server. MORE INFO: ${resBody.destination}`, 'log');
            // Create Response Object
            const responseObject = new ResponseBodyObject();
            // Build Response Object [status - transport]
            responseObject.status.transport = {
            ...
            // Console Log
            this.log(`ERROR: Unable To Transmit Data To:  ${resBody.destination}`, 'log');
            // Callback
            if (typeof resBody.callback === 'function') {
              resBody.callback(responseObject, resBody, portSpeaker);
            }
            ...
            // Console Log
            this.log(`NOTICE: Data Sent Successfully & Response Recieved From: ${resBody.destination}`, 'log');
            // Callback
            if (typeof resBody.callback === 'function') {
    ...
    const sendDataToGateway = () => {
      ...
      if (Object.values(portSpeaker.sockets).length === 0) {
        ...
        if (_connectionAttempts <= this.settings.microServiceConnectionTimeout) {
          ...
          // Notification
          this.log(`ERROR: Socket Initialization Timeout. PORT: ${portMicroServer}`, 'log');
        }
      } else {
        // Send Data To Destination
