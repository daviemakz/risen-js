...
class MSCommon {
  ...
  sendRequest(data, dest, kalive, options = { port: this.settings.apiGatewayPort }, callb) {
    // Get Variables
    ...
    const _portSpeaker = this.invokeSpeaker(options.port);
    // Build message Body
    const _resBody = {
      data: _data,
    ...
    const sendToSocket = () => {
      ...
      if (Object.values(_portSpeaker.sockets).length === 0) {
        ...
        responseObject.status.transport = {
          code: 2003,
          message: 'Unable To Connect To Service core Server',
        };
        // Build Response Object [status - transport]
        ...
        responseObject.resultBody.errData = {
          ...
          action: 'Connect To Service core Server',
          errorType: 'ERROR',
        ...
        _portSpeaker.request('COM_REQUEST', _resBody, _requestData => {
          ...
          if (_requestData.hasOwnProperty('error')) {
            ...
            responseObject.status.transport = {
              code: 2004,
              message: `Unable To Connect To Micro Server:  ${_resBody.destination}`,
            };
            // Build Response Object [status - transport]
            responseObject.status.command = {
              code: 200,
            ...
            responseObject.resultBody.errData = {
              entity: 'Client Request',
              action: `Connect To Micro Server:  ${_resBody.destination}`,
              errorType: 'ERROR',
              originalData: _resBody,
            };
            // Console Log
            ...
            // Console Log
            this.log(`Data sent successfully & response recieved from: ${_resBody.destination}`, 'log');
            // Callback
            if (typeof _resBody.callback === 'function') {
