...
class MSCommon {
  ...
  constructor(options) {
    // Bind methods
    return (
      ['log', 'invokeListener', 'invokeSpeaker', 'sendRequest', 'destroyConnection'].forEach(
      ...
      if (Object.values(_portSpeaker.sockets).length === 0) {
        ...
        _portSpeaker.request('COM_REQUEST', _resBody, _requestData => {
          ...
          if (_requestData.hasOwnProperty('error')) {
            ...
            const serviceExists = this.microServerData.hasOwnProperty(_resBody.destination);
            // Console Log
            serviceExists
              ? this.log(
                  `[${options.connectionId}] Service core has processed request for service: ${_resBody.destination}`,
                  'log'
                )
              : this.log(
                  `[${options.connectionId}] Service core was unable to find the service: ${_resBody.destination}`,
                  'log'
                );
            // Callback
            if (typeof _resBody.callback === 'function') {
