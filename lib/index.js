...
class ServiceCore extends MSCommon {
  ...
  processComRequest(data, message, id) {
    ...
    switch (true) {
      case _data.destination === coreName: {
        // Return
        return setTimeout(
          () =>
            this.coreOperations.hasOwnProperty(_data.funcName)
              ? this.coreOperations[_data.funcName](_data, _foreignSocket)
              : sentReplyToSocket(functionUnknown(data), _foreignSocket, false),
          0
        );
      }
