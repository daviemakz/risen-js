...
class ServiceCore extends MSCommon {
  ...
  getProcessIndex(name, port) {
    return this.microServerData[name].port.indexOf(port);
  }
  ...
  addServerToTracking(name, port) {
    ...
    if (this.microServerData.hasOwnProperty(name)) {
      return (
        (this.microServerData[name] = Object.assign(
          {},
          this.microServerData[name],
          {
            socket: this.microServerData[name].socket.concat(void 0), // void 0
            port: this.microServerData[name].port.concat(port),
            process: this.microServerData[name].process.concat(void 0),
            connectionCount: this.microServerData[name].connectionCount.concat(
              0
            ),
          }
        )) && true
  ...
  removeServerFromTracking(name, port) {
    ...
    // Add when instancisation is working
  }
      ...
      return new Promise(resolve => {
        const ensurePortFree = async () => {
          ...
          if (this.inUsePorts.includes(port)) {
            ...
            this.microServerData[name].process[
              ...
              `node ${__dirname}/server/index.js`,
              {
                ...
                env: {
                  ...
                  options: JSON.stringify(this.microServerOptions[name]),
                },
              },
              (error, stdout, stderr) => {
    ...
    const assignEventHandlers = () => {
      return new Promise(resolve => {
        ...
        ['stdout', 'stderr'].forEach(event =>
          this.microServerData[name].process[this.getProcessIndex(name, port)][
            event
  ...
  initConnectionToService(name, port, callback) {
    ...
    return this.initiateMicroServerConnection(port, socket => {
      ...
      // Store Socket Object
      this.microServerData[name].socket[
        this.getProcessIndex(name, port)
      ] = socket;
      // Callback
  ...
  microServerCommunication(recData, foreignSocket, localSocket, conId) {
    ...
    if (localSocket.status === 0) {
      ...
      ++this.microServerData[recData.destination].connectionCount[index];
      // Send to socket
  ...
  getMicroServiceSocket(name, socketList) {
    ...
    switch (true) {
      case typeof this.microServerOptions[name].loadBalancing === 'function': {
        // Must return [socket, index]
        return this.microServerOptions[name].loadBalancing(socketList);
      }
      case this.microServerOptions[name].loadBalancing === 'roundRobin': {
        // Queuing: random
        const socketIndex = this.microServerData[name].connectionCount.indexOf(
          Math.min(...this.microServerData[name].connectionCount)
        );
        // Queuing: roundRobin
        return [socketList[socketIndex], socketIndex];
      }
      case this.microServerOptions[name].loadBalancing === 'random': {
