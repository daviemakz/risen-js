...
class MicroServer extends MicroServiceCommon {
  ...
  constructor(options) {
    ...
    // Assign settings
    this.settings = process.env.settings;
  ...
  assignProcessListers() {
    return new Promise((resolve, reject) => {
      this.log('Assign process listeners', 'log');
      // exit
  ...
  initServer() {
    ...
    return new Promise((resolve, reject) => {
      ...
      isPortFree(process.env.port)
        ...
        .catch(e => {
          this.log(
            'ERROR: Micro Server Port Not Free Or Unknown Error Has Occurred. INFO: ' + JSON.stringify(e, null, 2),
            'log'
          );
          // Reject
          return reject(false);
...
new MicroServer(process.env.settings);
