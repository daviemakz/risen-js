...
class MicroServer extends MicroServiceCommon {
  ...
  assignOperations() {
    return new Promise(resolve => {
      ...
      Object.entries(require(process.env.operations)).forEach(([name, op]) => (this.operations[name] = op));
      // Assign standard functions
      this.standardFunctions.forEach(func => (this.operations[func] = this[func]));
      // Resolve promise
      return resolve();
    });
  }
      ...
      isPortFree(parseInt(process.env.port, 10))
        .then(() => {
          ...
          if (!this.interface) {
            ...
            // Return
            return reject(Error('Unable to start micro server!'));
          } else {
            // Console Log
  ...
  bindService() {
    return new Promise((resolve, reject) => {
      ...
      this.interface.on('MICRO_COM_REQUEST', (socket, data) => {
        // Confirm Connection
        this.log(`[${this.conId}] Micro service connection request recieved`);
