...
export const validateHttpOptions = httpOptions =>
  httpOptions.every(http => {
    switch (true) {
      ...
      case http.hasOwnProperty('ssl') && !['object', 'boolean'].includes(typeof http.ssl): {
...
export const validateCoreOperations = options =>
  options instanceof Object
    ? Object.entries(options).every(
        ([functionName, functionOp]) => typeof functionName === 'string' && typeof functionOp === 'function'
      )
    : false;
...
export const validateServiceOptions = serviceOption => {
  switch (true) {
    case (serviceOption.hasOwnProperty('runOnStart') && !Array.isArray(serviceOption.runOnStart)) ||
      (Array.isArray(serviceOption.runOnStart) && serviceOption.runOnStart.some(op => typeof op !== 'string')): {
